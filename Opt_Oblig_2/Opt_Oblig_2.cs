...
namespace Opt_Oblig_2 {
	class Opt_Oblig_2 {
		static void Main(string[] args) {
			...
			int unsatisfied_parent_1 = 100;
			int unsatisfied_parent_2 = 100;
			...
			while (counter < 1000 ) {
				...
				unsatisfied_parent_1 = FindFitnessUnsatisfied(n, first_parent, graph_of_connections);
			...
			if (unsatisfied_parent_1 < 1) {
				Console.WriteLine("optimal solution: ");
				PrintArray(n, first_parent);
			}
			else if (unsatisfied_parent_2 < 1) { 
				Console.WriteLine("optimal solution: ");
				PrintArray(n, second_parent);
			}
			else {
		...
		static void FillRandomGraph(int n, int[] max_connections, int[,] graph) {
			...
			for (int i = 0; i < n; i++) {
				...
				while (counter < amount_of_connections) {
					...
					random = rnd.Next(0, n);
					//Console.WriteLine("Amount" + amount_of_connections + " random " + random + " i " + i);
					//Console.WriteLine("connections " + max_connections[0] + max_connections[1] + max_connections[2] + max_connections[3] + max_connections[4] + counter);
				...
				if (i == 4) {
					for(int j = 0; j < n; j++) {
						if (max_connections[j] == 0) {
							teller++;
							//Console.WriteLine("connections: " + max_connections[j] + " teller " + teller + " j " + j);
						}
					}
