		...
		static void FillRandomGraph(int n, int[] max_connections, int[,] graph) {
			...
			for (int i = 0; i < n; i++) {
				...
				while (counter < amount_of_connections) {
					...
					Console.WriteLine("test" + counter + i);
					random = rnd.Next(0, n);

					//if a node already has max connections
					if (max_connections[i] == amount_of_connections || max_connections[i] == 3)
						counter = 99;
					//connect nodes as long as they haven't reached max connections and that they dont connect to itself
					else if (graph[i, random] != 1 && max_connections[i] != 3 && max_connections[random] != 3 && i != random) {
						/*if (graph[i, random] == 1)
							continue;
						*/
						graph[i, random] = 1;
